...
serve(async (req) => {
  ...
  try {
    const { messages, model = 'z-ai/glm-4.5-air:free', max_tokens = 1000, temperature = 0.7, reasoning = { enabled: true } }: ChatRequest = await req.json()
...
Always be helpful, professional, and mention that you're part of OmniPDF AI suite when appropriate. If users ask about the developer, credit Abir Hasan Siam.`
    ...
    if (!OPENROUTER_API_KEY) {
      console.error('❌ Edge Function: Missing OpenRouter API key')
      return new Response(
