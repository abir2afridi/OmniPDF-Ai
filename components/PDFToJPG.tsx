...
const PageCard: React.FC<PageCardProps> = ({ meta, selected, converted, onToggle, onDownload }) => (
    <motion.div
        ...
        className={`relative group cursor-pointer rounded-[5px] overflow-hidden border-2 transition-all duration-150 select-none
      ${selected
        ...
        <div className="relative bg-gray-100 dark:bg-white/5" style={{ aspectRatio: `${meta.width}/${meta.height}` }}>
            ...
            {converted && onDownload && (
                <button
                    ...
                    className="absolute bottom-1.5 right-1.5 opacity-0 group-hover:opacity-100 transition-opacity bg-black/70 hover:bg-black/90 text-white p-1 rounded-[5px]"
...
const FileSection: React.FC<FileSectionProps> = ({
    ...
    return (
        <div className="bg-white dark:bg-[#262636] rounded-[5px] border border-gray-100 dark:border-white/5 overflow-hidden">
            ...
            {(status === 'done' || status === 'converting') && (
                <div className="px-4 pb-4 pt-1">
                    <div className="flex items-center gap-2 px-3 py-2 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-[5px] focus-within:ring-2 focus-within:ring-yellow-400 transition-all">
...
export const PDFToJPG: React.FC<PDFToJPGProps> = ({ onBack, activeTool }) => {
        ...
        <div className="flex-1 flex flex-col h-full bg-[#f3f1ea] dark:bg-[#1e1e2e] overflow-hidden relative">
            ...
            <div className="shrink-0 flex items-center justify-between px-4 lg:px-6 py-4 bg-[#f3f1ea] dark:bg-[#262636] border-b border-gray-100 dark:border-white/5 shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <div className={`${activeTool?.toImageUrl ? 'h-8 w-auto px-1.5' : 'w-8 h-8'} rounded-[5px] flex items-center justify-center ${activeTool?.color || 'bg-yellow-500'} bg-opacity-10 dark:bg-opacity-20 overflow-hidden gap-1`}>
                        {activeTool?.imageUrl ? (
                            <>
                        ...
                        {files.length === 0 ? (
                            <div className="flex flex-col items-center justify-center gap-3">
                                <motion.div animate={{ y: [0, -5, 0] }} transition={{ repeat: Infinity, duration: 3, ease: 'easeInOut' }}
                                    className="p-4 bg-yellow-100 dark:bg-yellow-900/30 rounded-[5px] shadow-lg shadow-yellow-200 dark:shadow-yellow-900/30 mx-auto">
                        ...
                        <div className="p-5 border-b border-gray-100 dark:border-white/5">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <div className="flex items-center gap-1">
                                    <button onClick={() => setThumbSize(s => Math.max(60, s - 20))} className="p-1 hover:bg-gray-100 dark:hover:bg-white/5 rounded-[5px] transition-colors">
                                    ...
                                    <button onClick={() => setThumbSize(s => Math.min(240, s + 20))} className="p-1 hover:bg-gray-100 dark:hover:bg-white/5 rounded-[5px] transition-colors">
                                            ...
                                            <div className="grid grid-cols-3 gap-1.5">
                                                {([
                                                    ...
                                                    <button key={f.v} onClick={() => setFormat(f.v)}
                                                        className={`py-2 px-1 flex flex-col items-center text-[10px] font-bold rounded-[5px] border-2 transition-all
                                            ...
                                            <div className="grid grid-cols-2 gap-1.5">
                                                {([
                                                    ...
                                                    <button key={q.v} onClick={() => setQuality(q.v)}
                                                        className={`py-2 flex flex-col items-center text-[10px] font-bold rounded-[5px] border-2 transition-all
