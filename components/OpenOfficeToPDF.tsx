/**
 * OpenOfficeToPDF — Production-ready OpenDocument → PDF conversion module.
 *
 * Features:
 * - Multi-file drag & drop upload (.odt / .ods / .odp)
 ...
 * - Format detection badges (ODT/ODS/ODP)
 * - Advanced output settings (page size, orientation, quality)
 */

import React, { useState, useCallback, useRef } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import {
    Monitor, Upload, X, Download, Loader2, CheckCircle2,
...
interface ManagedFile {
    ...
    status: FileStatus;
    progress: number;
    result?: OOConversionResult;
    errorMsg?: string;
    customName: string;
    previewOpen: boolean;
    detectedFormat: OOFormat;
}
...
const ACCEPT = '.odt,.ods,.odp,application/vnd.oasis.opendocument.text,application/vnd.oasis.opendocument.spreadsheet,application/vnd.oasis.opendocument.presentation';

const statusColor: Record<FileStatus, string> = {
    idle: 'text-gray-400',
    converting: 'text-teal-500',
    done: 'text-emerald-500',
    error: 'text-red-500',
};

const formatBadge: Record<OOFormat, { label: string; bg: string; text: string; icon: React.ElementType }> = {
    odt: { label: 'ODT', bg: 'bg-blue-100 dark:bg-blue-900/30', text: 'text-blue-600 dark:text-blue-400', icon: FileText },
    ods: { label: 'ODS', bg: 'bg-green-100 dark:bg-green-900/30', text: 'text-green-600 dark:text-green-400', icon: FileSpreadsheet },
    odp: { label: 'ODP', bg: 'bg-orange-100 dark:bg-orange-900/30', text: 'text-orange-600 dark:text-orange-400', icon: Presentation },
};

function detectFormat(file: File): OOFormat {
...
const CircularProgress = ({ value, size = 36 }: { value: number; size?: number }) => {
    ...
    return (
        <svg width={size} height={size} className="-rotate-90">
            ...
            <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="currentColor"
                strokeWidth="3" strokeLinecap="round" className="text-teal-500"
...
const FileRow: React.FC<FileRowProps> = ({
    ...
    const badge = formatBadge[entry.detectedFormat];
            ...
            <div className="flex items-center gap-3 p-4">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-gray-400 font-mono">
                        ...
                        {entry.status === 'converting' && <span className="ml-2 text-teal-500">{entry.progress}%</span>}
                ...
                <div className="flex items-center gap-1 shrink-0">
                    ...
                    {entry.status === 'converting' && (
                        <div className="px-3 py-1.5 text-xs text-teal-500 font-bold flex items-center gap-1.5">
...
export const OpenOfficeToPDF: React.FC<OpenOfficeToPDFProps> = ({ onBack }) => {
    ...
    const addFiles = useCallback((incoming: FileList | File[]) => {
        ...
        for (const f of arr) {
            const err = validateOpenOfficeFile(f);
            ...
            next.push({
                ...
                status: 'idle',
                progress: 0,
                customName: f.name.replace(/\.(odt|ods|odp)$/i, ''),
                previewOpen: false,
    ...
    const convertOne = useCallback(async (id: string) => {
        ...
        try {
            const result = await convertOpenOfficeToPDF(entry.file, {
    ...
    const downloadOne = useCallback((entry: ManagedFile) => {
        ...
        const name = entry.customName.trim() || entry.file.name.replace(/\.(odt|ods|odp)/i, '');
    ...
    const downloadAll = useCallback(async () => {
        ...
        for (const entry of done) {
            const name = (entry.customName.trim() || entry.file.name.replace(/\.(odt|ods|odp)/i, '')) + '.pdf';
    ...
    // Format counts
    const odtCount = files.filter(f => f.detectedFormat === 'odt').length;
    const odsCount = files.filter(f => f.detectedFormat === 'ods').length;
    const odpCount = files.filter(f => f.detectedFormat === 'odp').length;
        ...
        <div className="flex-1 flex flex-col h-full bg-[#f3f1ea] dark:bg-[#1e1e2e] overflow-hidden relative">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[11px] text-gray-400 font-medium">
                            Convert .odt, .ods, and .odp files to PDF — in your browser
                        </p>
                    </div>
                </div>
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                <div className="flex-1 flex flex-col overflow-hidden p-4 lg:p-6 gap-4 min-w-0">
                    ...
                    >
                        ...
                        <motion.div
                            ...
                            className="p-4 bg-gradient-to-br from-teal-100 to-emerald-100 dark:from-teal-900/30 dark:to-emerald-900/30 rounded-2xl shadow-lg shadow-teal-200 dark:shadow-teal-900/30"
                        >
                            <Upload className="w-8 h-8 text-teal-600 dark:text-teal-400" />
                        </motion.div>
                        <div className="text-center">
                            <p className="text-base font-black dark:text-white">Drop OpenDocument files here</p>
                            ...
                            {files.length === 0 ? (
                                ...
                                >
                                    ...
                                    <p className="text-xs text-gray-300 dark:text-gray-600 max-w-xs">
                                        Drop OpenDocument files above or click to browse. Supports ODT (text), ODS (spreadsheet), and ODP (presentation) formats for batch conversion.
                                    </p>
                                </motion.div>
                            ) : (
                    ...
                    {files.length > 0 && (
                        <div className="shrink-0 flex items-start gap-2 px-4 py-3 bg-amber-50 dark:bg-amber-900/15 border border-amber-100 dark:border-amber-500/20 rounded-xl">
                            ...
                            <p className="text-[11px] text-amber-700 dark:text-amber-300 leading-relaxed">
                                ...
                                into styled HTML, then rendered to a PDF using <em>jsPDF + html2canvas</em>. Embedded images are preserved.
                                Complex formatting (macros, advanced charts) may render approximately.
                            </p>
                        </div>
                    )}
                </div>
                ...
                <div className="w-80 shrink-0 flex flex-col border-l border-gray-100 dark:border-white/5 bg-[#f3f1ea] dark:bg-[#262636] overflow-y-auto">
                        ...
                        <div className="p-5 border-b border-gray-100 dark:border-white/5">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {[
                                    ...
                                    { label: 'Pending', value: idleCount + errorCount, color: 'text-teal-600 dark:text-teal-400' },
                    ...
                    {/* Supported Formats */}
                    <div className="p-5 border-b border-gray-100 dark:border-white/5">
                        <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-3">Supported Formats</p>
                        <div className="space-y-2">
                            {[
                                { ext: '.odt', desc: 'OpenDocument Text (Writer)', icon: FileText, color: 'text-blue-500' },
                    ...
                    <div className="p-5 border-b border-gray-100 dark:border-white/5">
                        ...
                        <ul className="space-y-2">
                            {[
                                'Compatible with LibreOffice & OpenOffice files',
                                'Text, images, tables & lists are preserved',
                                'Spreadsheets render all sheets as pages',
                                'Presentations render slides individually',
                                ...
                                <li key={i} className="flex items-start gap-2 text-[11px] text-gray-500 dark:text-gray-400">
                                    <span className="w-4 h-4 shrink-0 rounded-full bg-teal-100 dark:bg-teal-900/30 text-teal-600 dark:text-teal-400 text-[8px] font-black flex items-center justify-center mt-0.5">{i + 1}</span>
                                    {tip}
                                </li>
                            ))}
                        </ul>
                    ...
                    <div className="p-5 border-t border-gray-100 dark:border-white/5 bg-white/80 dark:bg-[#262636]/80 backdrop-blur-sm space-y-3">
                        ...
                        {files.length === 0 && (
                            <p className="text-center text-[10px] text-gray-400">Upload OpenDocument files to begin</p>
                        )}
                    </div>
                </div>
            </div>
