...
interface PDFToExcelProps { onBack?: () => void; activeTool?: PDFTool; }
                        ...
                        <>
                            {entry.totalPages > 1 && (
                                <button onClick={() => setShowRange(v => !v)} title="Page range"
                                    className="p-1.5 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-[5px] text-green-400 hover:text-green-600 transition-colors">
                                    <Settings2 className="w-4 h-4" />
                                </button>
                            )}
                            <button onClick={onConvert} disabled={isAnyConverting}
                                className="px-3 py-1.5 bg-green-600 hover:bg-green-500 text-white text-xs font-bold rounded-[5px] transition-colors disabled:opacity-40 disabled:cursor-not-allowed shadow-sm">
                                Convert
                            </button>
                        </>
                    )}
                    ...
                    {entry.status === 'done' && (
                        <>
                            ...
                            <button onClick={onDownload}
                                ...
                                <Download className="w-3.5 h-3.5" /> <span className="hidden sm:inline">.xlsx</span>
                            </button>
                        </>
                    )}
                    {entry.status === 'error' && (
                        ...
                        <div className="px-4 pb-4 pt-2 border-t border-gray-50 dark:border-white/5">
                            ...
                            <div className="flex items-center gap-2">
                                <input type="text" value={rangeVal} onChange={e => setRangeVal(e.target.value)}
                                    ...
                                    className="flex-1 px-3 py-2 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-[5px] text-xs font-mono dark:text-white outline-none focus:ring-2 focus:ring-green-400" />
                                <button onClick={() => { onPageRangeChange(rangeVal); setShowRange(false); }}
                                    className="px-3 py-2 bg-green-600 text-white text-xs font-bold rounded-[5px] hover:bg-green-500">
                                    Apply
                                </button>
                                <button onClick={() => { setRangeVal(''); onPageRangeChange(''); }}
                                    className="px-3 py-2 bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-400 text-xs font-bold rounded-[5px] hover:bg-gray-200 dark:hover:bg-white/10">
                                    All
                                </button>
                            </div>
...
export const PDFToExcel: React.FC<PDFToExcelProps> = ({ onBack, activeTool }) => {
                ...
                <div className="flex items-center gap-3">
                    {onBack && (
                        <button onClick={onBack}
                            className="p-2 hover:bg-gray-100 dark:hover:bg-white/5 rounded-[5px] transition-colors text-gray-500">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-gray-400 font-medium truncate">Extract tables & data from PDF → editable .xlsx</p>
                    </div>
                </div>
                <div className="flex items-center gap-1 lg:gap-2">
                    {doneCount > 1 && (
                        <button onClick={downloadAll}
                            className="px-3 py-2 text-xs font-bold text-green-600 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-[5px] flex items-center gap-1.5 transition-colors">
                    ...
                    {readyCount > 1 && (
                        <button onClick={convertAll} disabled={isConverting}
                            className="px-4 py-2 bg-green-600 hover:bg-green-500 disabled:opacity-40 text-white text-xs font-bold rounded-[5px] transition-colors shadow-sm flex items-center gap-1.5">
            ...
            <div className="flex-1 flex flex-col lg:flex-row overflow-y-auto lg:overflow-hidden">
              ...
              hover:border-green-400 dark:hover:border-green-500/50 hover:bg-green-50/30 dark:hover:bg-green-900/10`}>
                        ...
                        <motion.div animate={{ y: [0, -6, 0] }} transition={{ repeat: Infinity, duration: 3, ease: 'easeInOut' }}
                            className="p-4 bg-green-100 dark:bg-green-900/30 rounded-[5px] shadow-lg shadow-green-200 dark:shadow-green-900/30">
                ...
                <div className="lg:w-72 w-full shrink-0 flex flex-col border-l-0 lg:border-l border-gray-100 dark:border-white/5 bg-[#f3f1ea] dark:bg-[#262636] lg:overflow-y-auto">
                        ...
                        <div className="grid grid-cols-2 gap-1.5">
                            {([
                                ...
                                <button key={String(opt.v)} onClick={() => setOneSheetPerPage(opt.v)}
                                    className={`flex flex-col items-center gap-1.5 py-3 px-2 rounded-[5px] border-2 transition-all text-[10px] font-bold
                    ...
                    <div className="p-5 border-t border-gray-100 dark:border-white/5 sticky bottom-0 z-10">
                        <div className="flex items-center gap-3 p-3 bg-green-50 dark:bg-green-900/20 rounded-[5px]">
                            <div className="w-8 h-8 shrink-0 rounded-[5px] bg-green-600 flex items-center justify-center">
